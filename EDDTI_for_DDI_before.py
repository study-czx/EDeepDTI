...
def Get_drug_embedding(data_type):
    emb_feature_path_dr = dataset_base + 'feature/' + data_type + '/'
    chemberta = np.loadtxt(emb_feature_path_dr + 'ChemBERTa2.csv', dtype=float, delimiter=',')
    # np.random.shuffle(chemberta)
...
def Get_sample(DTI, N_DTI, dr_id_map):
    P_list, N_list = [],[]
    P_label, N_label = [],[]
    for i in range(len(DTI)):
        P_list.append([dr_id_map[DTI[i][0]], dr_id_map[DTI[i][1]]])
        P_label.append([1])
    for j in range(len(N_DTI)):
        N_list.append([dr_id_map[N_DTI[j][0]], dr_id_map[N_DTI[j][1]]])
        N_label.append([0])
...
for dataset in datasets:
    Drug_id_data = pd.read_csv(dataset_base + dataset + '_'+input_type+'/drug_smiles.csv')
    Drug_id = Drug_id_data['drug_id']
    # print(len(Drug_id))
    # get id map and features
    dr_id_map = funcs.id_map(Drug_id)
    # print(dr_id_map['DB01284'])
    ...
    for k in range(fold_number):
        ...
        load_base = dataset_base + dataset + "_"+input_type+"/" + fold_type
        train_P = np.loadtxt(load_base + "/train_P.csv", dtype=str, delimiter=",",skiprows=1)
        train_N = np.loadtxt(load_base + "/train_N.csv", dtype=str, delimiter=",",skiprows=1)
        dev_P = np.loadtxt(load_base + "/dev_P.csv", dtype=str, delimiter=",",skiprows=1)
        dev_N = np.loadtxt(load_base + "/dev_N.csv", dtype=str, delimiter=",",skiprows=1)

        test_P = np.loadtxt(load_base + "/test_P.csv", dtype=str, delimiter=",", skiprows=1)
        test_N = np.loadtxt(load_base + "/test_N.csv", dtype=str, delimiter=",", skiprows=1)
        ...
        train_X, train_Y = Get_sample(train_P, train_N, dr_id_map)
        dev_X, dev_Y = Get_sample(dev_P, dev_N, dr_id_map)
        test_X, test_Y = Get_sample(test_P, test_N, dr_id_map)
