...
- For EDeepDTI, input_type = 'e'; for EDeepDTI-d, input_type = 'd'.<br>
...
- Run `view_baseline_results/get_results_table.py` to obtain the prediction results of EDeepDTI, EDeepDTI-d, EDeepDTI-s, and all baseline methods. The results can be found in `view_baseline_results/1_all_results_tables`.<br>
- Run `view_baseline_results/cal_t_value.py` to obtain the p-values of the t-tests between EDeepDTI and all baseline methods. The results can be found in `view_baseline_results/1_all_p_values`.<br>
- For grid search for hyperparameters, run `EDeepDTI_GridSearch.py` to determine the values of hyperparameters, run `EDeepDTI_GridSearch_epoch.py` to determine the number of epochs.(`EDeepDTI_all_records.csv`) <br>
 ...
 * ProtTrans (ProtTrans/)  
（1）Run `embedding_proteins.py` to generate protein prott5 features for DrugBank, CPI, Davis, and KIBA datasets.
 * ESM2 (ESM2/)  
（1）Run `embedding_proteins_esm.py` to generate protein esm2 features for DrugBank, CPI, Davis, and KIBA datasets.

#### Drug fingerprints and protein sequence descriptors (datasets_DTI/)  
（1）Run `cal_drug_structure.py` to calculate the MACCS, ECFP4, and FCFP4 fingerprints for DrugBank, CPI, Davis, and KIBA dataset.<br>
...
#### Drug and protein similarity features (datasets_DTI/)  
（1）In `cal_feature_sim.py`, run 'cal_finger_sim(data_types)' to calculate Jaccard similarity measures of five molecular fingerprints for DrugBank, Davis, and KIBA datasets; run 'cal_DDI_sim()' to calculate Jaccard similarity measures of DDIs for DrugBank dataset.<br>
（2）In `cal_feature_sim.py`, run 'cal_PPI_sim(data_types)' to calculate Jaccard similarity measures of PPIs, and the topological similarities of the human PPI network for DrugBank, CPI, Davis, and KIBA datasets.<br>
（3）Run `cal_GO_seq_sim.R` to calculate protein sequence similarity and three types of GO semantic similarities for DrugBank, CPI, Davis, and KIBA datasets.<br>
...
（4）Run `get_top_end_drugs.py` to get top and bottom 30 predicted drugs, and known interacting drugs of selected proteins.<br>
...
## Citation
Cheng, Z., Yan, Q., Ding, D. et al. A Scalable and Robust Ensemble Deep Learning Method for Predicting Drug-Target Interactions. Interdiscip Sci Comput Life Sci (2025). https://doi.org/10.1007/s12539-025-00774-8
