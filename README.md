...
Accurate identification of drug-target interactions (DTIs) is crucial for drug discovery, significantly reducing development time and costs. In this study, we propose EDeepDTI, an ensemble deep learning framework designed to leverage multi-view features to enhance the generalization of DTI predictions. EDeepDTI utilizes pre-trained models to learn rich structural and semantic representations of drugs and proteins and trains multiple independent deep learning base learners on these features. The predictions from these base learners are then aggregated to improve the performance. Additionally, we propose two model variants (EDeepDTI-d and EDeepDTI-s) with different input features to demonstrate the robustness of EDeepDTI, and three variants (EDeepDDI, EDeepPPI, and EDeepMDA) extended to predict different types of biological interactions to showcase its scalability. 
  </p>
