...
def extract_features(config_name, model_path, data_path, dataset):
    fps_list = []
    ...
    mol_dataset = MoleculeDataset(root_path=data_path, dataset = dataset, dataset_type=None)
    loader = DataLoader(mol_dataset, batch_size=16, shuffle=False, num_workers=0, drop_last=False, collate_fn=collator)
    model = LiGhT(
    ...
    # kpgt_emb.to_csv(f"{data_path}/{dataset}/kpgt_emb_max.csv", index=False, header=False)
    # np.savez_compressed(f"{data_path}/{dataset}/kpgt.npz", fps=np.array(fps_list))
    print(f"The extracted features were saving at {data_path}/{dataset}/kpgt_emb.csv")
...
if __name__ == "__main__":
    ...
    for type in types:
        ...
        smiles = smiles_list[['smiles']]
        # smiles.columns = ['smiles']
        preprocess_dataset(base_path, smiles, type)

        extract_features('base', 'models/pretrained/base/base.pth', 'data/', type)

# os.system('python preprocess_downstream_dataset.py --data_path ../datasets/ --dataset bace ')
# os.system('python extract_features.py --config base --model_path ../models/pretrained/base/base.pth --data_path ../datasets/ --dataset bace')
