...
for dataset in datasets:
    ...
    model_save_path_base = 'models_PPI2/' + dataset
    ...
    for k in range(fold_number):
        ...
        train_P = np.concatenate((train_P, dev_P))
        train_N = np.concatenate((train_N, dev_N))
        ...
        print('number of DDI: ', len(train_P), len(test_P))
        print('number of Negative DDI ', len(train_N), len(test_N))
        ...
        for m in range(n_p_feats):
            for n in range(n_p_feats):
                ...
                for epoch in range(num_epoches):
                    ...
                    print('epoch:{},Train Loss: {:.4f},Train Auc: {:.4f}, '.format(epoch, train_avloss, train_auc))
                torch.save(model.state_dict(), model_save_path + '/model' + str(model_number) + '.pt')
                    # valid
