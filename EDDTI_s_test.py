...
dataset = 'DTI'
predict_types = ['5_fold', 'new_drug_protein']
# input type: s
input_type = 's'
...
for predict_type in predict_types:
    model_save_path_base = 'models_test/' + save_base + '/' + dataset + '/' + predict_type
        ...
        for m in range(n_dr_feats):
            for n in range(n_p_feats):
                ...
                for epoch in range(num_epoches):
                    ...
                    with torch.no_grad():
                        ...
                        if dev_auc >= best_auc:
                            ...
                            torch.save(model.state_dict(), model_save_path + '/model' + str(model_number) + '.pth')
                if model_number == '0':
                ...
                test_model.load_state_dict(
                    torch.load(model_save_path + '/model' + str(model_number) + '.pth'))
                with torch.no_grad():
    ...
    # all_output_results.to_csv('all_records.csv', index=False)
